...
abstract class AbstractCommand {
    ...
    /**
     *
     * @var type CommandsFactory
     */
    protected $commsFactory;

    public function __construct(Publisher $pubsub, CommandsFactory $commsFactory) {
        $this->pubsub = $pubsub;
    ...
    public static function fit(Update $up) {
        $text = NULL;
        if ($up->getCommand())
            $text = $up->getCommand()->getName();
        elseif ($up->getMessage() && $up->getMessage()->getText())
            $text = $up->getMessage()->getText();
